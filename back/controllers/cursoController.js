...
const updateCurso = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { body } = req;
    let curso = await CursoService.updateCurso(id, body);
  ...
  try {
    ...
    for (const trilha of curso.trilhas) {
      ...
      for (const conteudo of trilhaInfo.conteudos) {
        let conteudoInfo = await ConteudoService.getById({
          _id: conteudo.toString(),
        });
        let conteudoReturn;
        conteudoReturn = {
          id: conteudoInfo._id,
          nome: conteudoInfo.titulo,
          check: false,
...
module.exports = { create, addTrilha, getAll, getById, updateCurso };
